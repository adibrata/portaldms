(function() {
    ...
    var db = {
        ...
        loadData: function(filter) {
            return $.grep(this.documents, function(documents) {
                return (!filter.Customer || documents.Customer.indexOf(filter.Customer) > -1)
                    && (!filter.Project || documents.Project.indexOf(filter.Project) > -1)
                    && (!filter.Require || documents.Require === filter.Require)
                    && (!filter.Upload || documents.Upload === filter.Upload)
                    && (!filter.Uncomplete || documents.Uncomplete === filter.Uncomplete)
            });
        },
		
		 insertItem: function(insertingdocuments) {
            this.documents.push(insertingdocuments);
        },

        updateItem: function(updatingdocuments) { },

        deleteItem: function(deletingdocuments) {
            var documentsIndex = $.inArray(deletingdocuments, this.documents);
            this.documents.splice(documentsIndex, 1);
        },
